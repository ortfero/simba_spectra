...
namespace ufmt {
    ...
    private:
        ...
        template<typename T> static fixed_string from_integer(T n) noexcept {
            auto s  = fixed_string{};
            auto [ptr, ec] = std::to_chars(s.p_, s.p_ + N, n);
            *ptr = '\0';
            s.resize(ptr - s.p_);
            return s;
        }
    ...
    public:
        ...
        static fixed_string from_number(int32_t n) noexcept {
            return from_integer<int32_t>(n);
        }

        static fixed_string from_number(uint32_t n) noexcept {
            return from_integer<uint32_t>(n);
        }

        static fixed_string from_number(int64_t n) noexcept {
            return from_integer<int64_t>(n);
        }

        static fixed_string from_number(uint64_t n) noexcept {
            return from_integer<uint64_t>(n);
        }
